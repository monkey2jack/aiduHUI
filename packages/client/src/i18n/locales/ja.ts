...
export default {
  ...
  browser: {
    title: 'ブラウザー', settings: 'ブラウザー設定', desktopOnly: '内蔵ブラウザーは aiduHUI Desktop でのみ利用できます。', newTab: '新しいタブ',
  ...
  agentManager: {
    ...
    legacyDataMigrationSuccess: '移行を予約しました。aiduHUI を再起動し、ローカルサービスの開始前にデータを移行します。',
  ...
  runtimeVersions: {
    ...
    dataDirectoryEnvRestart: '変更後は aiduHUI を完全に終了して再度開いてください。Windows または Linux のデスクトップアプリで古い値が残る場合は、サインアウトして再度サインインしてください。',
    cliUpdateDescription: 'Hermes Runtime 0.19.1 以降では、次のコマンドで内蔵 Hermes Agent のみを更新できます。aiduHUI デスクトップアプリや Web UI は更新されません。実行前に aiduHUI を完全に終了してください：',
    webUiTitle: 'Web UI',
    platform: 'プラットフォーム',
    ...
    runtimeDirectorySaved: 'Runtime の移行を予約しました。aiduHUI を再起動すると実行されます。',
    runtimeMigrationPending: '移行先：{directory}',
    runtimeMigrationFailed: '保存先の移行に失敗したため、以前の保存先を引き続き使用します',
    runtimeActivationFailed: '選択した Runtime を起動できなかったため、aiduHUI は別のインストール済みバージョンを使用しています',
    active: '有効',
    installed: 'インストール済み',
    download: 'ダウンロード',
    ...
    activateSuccess: 'バージョン選択を保存しました。aiduHUI を再起動すると反映されます。',
  ...
  connections: {
    ...
    app: {
      ...
      downloadTitle: 'スマートフォンで aiduHUI を使う',
      downloadDescription: '現在クローズドベータ中です。登録には登録コードが必要で、今後ソーシャルメディアで一部のコードを配布します。',
      downloadPaidDescription: 'aiduHUI App は正式にリリースされ、現在は有料でご利用いただけます。ご利用になるには、料金・購入ページで最適なプランを選び、購入手続きを完了してください。',
      ...
      accessFailures: {
        ...
        cloudSubscriptionRequired: 'このアカウントには aiduHUI Cloud の利用権限がありません',
  ...
  chat: {
    ...
    pushNotConfigured: '有効にする前に、aiduHUI の「デバイス接続」でメッセージプッシュを設定してください。',
  ...
  settings: {
    ...
    voice: {
      ...
      hermesConfigDescription: 'Hermes プロファイルの既存ネイティブプロバイダーは保持され、Web UI のプロバイダーは aiduHUI として表示されます。',
      ...
      useHermesStudioProvider: 'aiduHUI プロバイダーを使用',
  ...
  changelog: {
    new_0_7_19_1: '名称を aiduHUI に変更し、アプリアイコン、公式ドメイン、CLI コマンド、製品ドキュメントを統一 (#2966, #2980, #2981, #2992, #2994)',
  ...
  workbench: {
    navLabel: 'ワークベンチのナビゲーション',
    sectionModels: 'モデル管理',
    sectionModelsHint: '汎用 · 補助 · 組み合わせ',
    sectionSettings: '設定',
    sectionSettingsHint: '圧縮 · 性能 · 使用量 · ログ',
    sectionJobs: 'ジョブ',
    sectionJobsHint: 'スケジュールと実行履歴',
    sectionChannels: 'チャンネル',
    sectionChannelsHint: 'プラットフォーム連携',
    sectionMemory: 'メモリ',
    sectionMemoryHint: '長期メモリ',
    paneCompression: 'コンテキスト圧縮',
